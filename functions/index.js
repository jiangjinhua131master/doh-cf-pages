...
export async function onRequest({ request, waitUntil }) {
  ...
  if (!isDnsQuery && (path === '/' || path === '')) {
    ...
    <head>
        ...
        <title>专用 DoH DNS 服务 (Pages 架构完全体)</title>
        <style>
    ...
    <body>
        <div class="container">
            ...
            <h1>专用 DoH DNS 服务</h1>
            <div class="status-tag"><span class="dot"></span>七星 Anycast 弹性集群 (CF Pages 托管)</div>
            
            <div class="info-box">
                <div class="info-item"><span style="color:#8b949e">入口地址:</span><span class="value" style="font-weight:bold;color:#58a6ff;">${url.origin}/dns-query</span></div>
  ...
  if (isDnsQuery) {
    ...
    // A. 第一防线：Cloudflare 真正边缘 Cache 检索
    ...
    // C. 第二防线：尝试单发主用节点（带有 800ms 超时熔断保护）
    if (preferredUpstream && SPEED_RACE_UPSTREAMS.includes(preferredUpstream)) {
      try {
        ...
        preferredUpstream = null; // 触发 QPS 限速或抖动降级，跌落至并发洗牌
      }
    }
    ...
    if (!finalDnsResult) {
      ...
      try {
        ...
        waitUntil(cache.put(new Request(upstreamCacheKey), saveBestUpstreamResponse));
    ...
    try {
      ...
      if (contentType.includes('json')) {
        ...
        waitUntil(cache.put(new Request(cacheKeyUrl.toString(), { method: 'GET' }), cacheResponse.clone()));
